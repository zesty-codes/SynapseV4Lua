...
local function disassemble(a1, showOps)
    ...
    local function readProto(proto, depth)
        ...
        for i = 1,proto.numParams do
            addTabSpace(depth);
            output = output .. string.format("local l__syn%i__i = arg%i\n", i - 1, i - 1);
        end
        ...
        while codeIndex < proto.sizeCode do
            ...
            if markedAux then
                ...
                if opc == getOpCode("LOADNIL") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = nil", A)
                elseif opc == getOpCode("BREAK") then
                    output = output .. "break"
                elseif opc == getOpCode("LOADK") then
                    local k = proto.kTable[Bx + 1] or nilValue;
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = %s", A, (type(k.value) == "string") and ("\"" .. k.value .. "\"") or tostring(k.value))
                elseif opc == getOpCode("LOADKX") then
                    local k = proto.kTable[aux + 1] or nilValue;
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = %s", A, (type(k.value) == "string") and ("\"" .. k.value .. "\"") or tostring(k.value))
                elseif opc == getOpCode("LOADB") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = %s", A, tostring(B == 1))
                elseif opc == getOpCode("LOADN") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = %s", A, tostring(Bx))
                elseif opc == getOpCode("GETUPVAL") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = upvalues[%i]", A, B)
                elseif opc == getOpCode("SETUPVAL") then
                    output = output .. string.format("upvalues[%i] = l__syn%i__i", B, A)
                elseif opc == getOpCode("MOVE") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i", A, B)
                elseif opc == getOpCode("LEN") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = #l__syn%i__i", A, B)
                elseif opc == getOpCode("UNM") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = -l__syn%i__i", A, B)
                elseif opc == getOpCode("NOT") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = not l__syn%i__i", A, B)
                elseif opc == getOpCode("GETVARARGS") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = ...", A)
                elseif opc == getOpCode("CONCAT") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i .. l__syn%i__i", A, B, C)
                elseif opc == getOpCode("AND") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i and l__syn%i__i", A, B, C)
                elseif opc == getOpCode("OR") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i or l__syn%i__i", A, B, C)
                elseif opc == getOpCode("ANDK") then
                    local k = proto.kTable[C + 1] or nilValue;
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i and %s", A, B, tostring(k.value))
                elseif opc == getOpCode("ORK") then
                    local k = proto.kTable[C + 1] or nilValue;
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i or %s", A, B, tostring(k.value))
                elseif opc == getOpCode("GETIMPORT") then
                    ...
                    if indexCount == 1 then
                        ...
                        output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = %s", A, tostring(k1.value))
                    elseif indexCount == 2 then
                        local k1 = proto.kTable[cacheIndex1 + 1];
                        local k2 = proto.kTable[cacheIndex2 + 1];
                        
                        output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = %s[\"%s\"]", A, k1.value, tostring(k2.value))
                    elseif indexCount == 3 then
                        local k1 = proto.kTable[cacheIndex1 + 1];
                        local k2 = proto.kTable[cacheIndex2 + 1];
                        local k3 = proto.kTable[cacheIndex3 + 1];
                        
                        output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = %s[\"%s\"][\"%s\"]", A, k1.value, tostring(k2.value), tostring(k3.value))
                    else
                        error("[GETIMPORT] Too many entries");
                    end
                elseif opc == getOpCode("GETGLOBAL") then
                    local k = proto.kTable[aux + 1] or nilValue;
                    output = output .. string.format("l__syn%i__i = %s", A, tostring(k.value))
                elseif opc == getOpCode("SETGLOBAL") then
                    local k = proto.kTable[aux + 1] or nilValue;
                    output = output .. string.format("%s = l__syn%i__i", tostring(k.value), A)
                elseif opc == getOpCode("GETTABLE") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i[l__syn%i__i]", A, B, C)
                elseif opc == getOpCode("SETTABLE") then
                    output = output .. string.format("l__syn%i__i[l__syn%i__i] = l__syn%i__i", B, C, A)
                elseif opc == getOpCode("GETTABLEN") then
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i[%i]", A, B, C - 1)
                elseif opc == getOpCode("SETTABLEN") then
                    output = output .. string.format("l__syn%i__i[%i] = l__syn%i__i", B, C - 1, A)
                elseif opc == getOpCode("GETTABLEKS") then
                    local k = proto.kTable[aux + 1] or nilValue;
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i[%s]", A, B, (type(k.value) == "string") and ("\"" .. k.value .. "\"") or tostring(k.value))
                elseif opc == getOpCode("SETTABLEKS") then
                    local k = proto.kTable[aux + 1] or nilValue;
                    output = output .. string.format("l__syn%i__i[%s] = l__syn%i__i", B, (type(k.value) == "string") and ("\"" .. k.value .. "\"") or tostring(k.value), A)
                ...
                elseif opc == getOpCode("ADDK") then
                    local k = proto.kTable[C + 1] or nilValue;
                    output = output .. (isVarDefined(A) and "" or "local ") .. string.format("l__syn%i__i = l__syn%i__i + %s", A, B, tostring(k.value));
                ...
                elseif opc == getOpCode("CALL") then
                    if C > 1 then
                        for j = 1, C - 1 do
                            output = output .. string.format("l__syn%i__i", A + j - 1)
                            if j < C - 1 then output = output .. ", " end
                        end
                        output = output .. " = "
                    elseif C == 0 then
                        output = output .. string.format("l__syn%i__i, ...", A);
                        output = output .. " = "
                    ...
                    if B > 1 then
                        if nameCall then
                            for j = 1, B - 2 do
                                output = output .. string.format("l__syn%i__i", A + 1 + j) -- exclude self
                                if j < B - 2 then output = output .. ", " end
                            end
                        else
                            for j = 1, B - 1 do
                                output = output .. string.format("l__syn%i__i", A + j)
                                if j < B - 1 then output = output .. ", " end
                            end
                        end
                    elseif B == 0 then
                        output = output .. string.format("l__syn%i__i, ...", A + 1);
                        --for j = 1, proto.maxStackSize do
                        --    if nameCall then
                        --        output = output .. string.format("l__syn%i__i", A + 1 + j) -- exclude self
                        --    else
                ...
                elseif opc == getOpCode("SETLIST") then
                    ...
                    for j = 1, C do
                        addTabSpace(depth);
                        output = output .. string.format("l__syn%i__i[%i] = l__syn%i__i\n", A, j + fieldSize - 1, B + j - 1);
                    end
                ...
                elseif opc == getOpCode("NEWCLOSURE") then
                    ...
                    for j = codeIndex + 1, proto.sizeCode do
                        if luau.GET_OPCODE(proto.codeTable[j]) ~= getOpCode("CAPTURE") then
                            ...
                            if captureType == 0 or captureType == 1 then
                                output = output .. string.format("-- V nested upvalues[%i] = l__syn%i__i\n", upvalueIndex, captureIndex)
                    ...
                    if nextProto.source then
                        ...
                        addTabSpace(depth);
                        output = output .. string.format("l__syn%i__i = ", A) .. nextProto.source
                    else
                        nextProto.source = nil;
                        output = output .. string.format("l__syn%i__i = ", A) .. readProto(nextProto, depth)
                    end
...
local old = decompile
getgenv().decompile = function(v)
    local bytecode = getscriptbytecode(v)
    local success, output = pcall(disassemble, bytecode)
    local success1, output1 = pcall(old, v)
    if success then
        return "-- Decompiled with the Synapse X Luau decompiler.\n"..output
    elseif not success and success1 == true then
        return "-- Decompiled with the Synapse X Luau decompiler.\n"..old(v)
    end
    return "-- Failed to decompile the script "..v:GetFullName()
end
